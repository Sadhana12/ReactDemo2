...
import { LOAD_STR_SUCCESS, LOAD_STR, LOAD_STR_ERROR, ADD_NEW_STR, CHANGE_STRING} from './constants';
...
const appReducer = (state = initialState, action) =>
  produce(state, draft => {
    switch (action.type) {
      ...
      case ADD_NEW_STR:
        draft.strData.repositories = [{id: Math.max(...state.strData.repositories.map(item => item.id)) + 1, text: state.newStr}, ...state.strData.repositories]
        draft.newStr = '';
        break;
